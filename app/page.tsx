...
const BRAND = {
  ...
  tagline: "AI-powered email security for small and medium businesses.",
  phone: "(555) 987-6543",
...
const PRICING = [
  { name: "Starter", price: "$49/mo", features: ["Up to 25 mailboxes", "AI phishing & malware detection", "Basic quarantine & alerts"] },
  { name: "Pro", price: "$149/mo", features: ["Up to 100 mailboxes", "Advanced AI + anomaly detection", "Slack & Email alerts", "API access"] },
    ...
    <div className="min-h-screen w-full bg-slate-900 text-white">
        ...
        <div className="max-w-7xl mx-auto px-4 py-24 grid md:grid-cols-2 gap-12 items-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <div className="mt-6 flex flex-col sm:flex-row gap-3">
              <a href="#contact" className="px-6 py-3 rounded-2xl shadow font-semibold text-white" style={{ background: BRAND.primary }}>Start free trial</a>
              <a href="#features" className="px-6 py-3 rounded-2xl border border-slate-600 hover:shadow font-semibold">See features</a>
            </div>
            <div className="mt-6 flex items-center gap-4 text-sm text-gray-300">
              <div className="flex -space-x-2">
                <img src="https://placehold.co/100x100" className="h-8 w-8 rounded-full border-2 border-white" alt="customer"/>
                <img src="https://placehold.co/100x100" className="h-8 w-8 rounded-full border-2 border-white" alt="customer"/>
                <img src="https://placehold.co/100x100" className="h-8 w-8 rounded-full border-2 border-white" alt="customer"/>
              </div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }}>
            <div className="aspect-[4/3] bg-slate-800 border border-slate-700 rounded-3xl shadow p-3">
              <img src="https://placehold.co/1200x800" alt="Email security dashboard" className="w-full h-full object-cover rounded-2xl" />
            </div>
          </motion.div>
        </div>
      </section>
      ...
      <section id="features" className="max-w-7xl mx-auto px-4 py-20">
        <div className="flex items-end justify-between gap-4">
          ...
          <a href="#contact" className="hidden md:inline-flex text-sm font-semibold" style={{ color: BRAND.primary }}>Need something custom?</a>
        </div>
        <div className="mt-10 grid md:grid-cols-4 gap-6">
          {FEATURES.map((f, i) => (
            <motion.div key={f.title} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.05 }}
              ...
              <a href="#contact" className="mt-4 inline-flex items-center gap-2 text-sm font-semibold" style={{ color: BRAND.primary }}>
          ...
          <div className="mt-10 grid md:grid-cols-3 gap-6">
            {PRICING.map((p, i) => (
              <motion.div key={p.name} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.05 }}
                ...
                <a href="#contact" className="mt-6 inline-block text-center rounded-2xl px-4 py-2 font-semibold text-white" style={{ background: BRAND.primary }}>
                  Choose {p.name}
                </a>
              </motion.div>
            ))}
      ...
      <section id="about" className="bg-slate-900">
        <div className="max-w-7xl mx-auto px-4 py-20 grid md:grid-cols-2 gap-12 items-center">
          <div className="aspect-[4/3] bg-slate-800 border border-slate-700 rounded-3xl shadow p-3 order-last md:order-first">
            <img src="https://placehold.co/1200x800" alt="Security operations" className="w-full h-full object-cover rounded-2xl" />
          </div>
          <div>
      ...
      <section id="contact" className="bg-slate-900">
        <div className="max-w-7xl mx-auto px-4 py-20 grid md:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <p className="mt-3 text-gray-300">Tell us about your email environment and we’ll schedule a guided walkthrough.</p>
